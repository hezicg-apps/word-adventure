...
function loadFromLocal() {
    ...
    if (savedWords) {
        ...
        // אם יש מילים בזיכרון, עוברים ישר לכרטיסיות
...
function processInput() {
    ...
    // חילוץ כותרת אם קיימת (שורה ראשונה בלי מקף)
    if (!lines[0].includes('-')) {
...
window.onload = () => {
    ...
    if (sharedWords) {
        try {
            ...
            if (lines.length > 0) {
                // חילוץ כותרת מהשורה הראשונה אם אין בה מקף
                if (!lines[0].includes('-')) {
                    state.listName = lines[0];
                    const wordLines = lines.filter(l => l.includes('-'));
                    state.words = wordLines.map(l => {
        ...
        } catch(e) { console.error("URL W-Data Error", e); }
    } else if (sharedData) {
        try {
